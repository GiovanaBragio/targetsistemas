...
function calculateMetrics(revenueData: DailyRevenue[]): void {
    const validDays = revenueData.filter((day) => day.valor > 0);
    const values = validDays.map((day) => day.valor);
    ...
    console.log(`Days with revenue above average: ${daysAboveAverage}`);
}
